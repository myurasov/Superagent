...
- An **asset** marked `status: disposed` → moved to `assets-index.yaml.disposed[]` (kept in the live index for tax / insurance history); the corresponding `Resources/` sub-folder is moved to `Archive/` (the canonical source documents under `Sources/<your-folders>/` — e.g. `Sources/Vehicles/<asset-slug>/` — stay where they are — they're immutable per the Sources contract; user moves them manually if desired).
