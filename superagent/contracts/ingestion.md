...
- **Registry.** A source is one reference file, `Sources/Watchlist/<id>.ref.md` (folder overridable via `config.preferences.watchlist.path`), carrying the usual ref frontmatter plus a `watch:` block. The filename stem is the source id, its state key and its handle (`watch:<id>`). There is no Python table of sources and no central YAML registry.
...
- **`cycles`** on the ref (default `config.preferences.watchlist.cycles`, normally `[daily-update]`) says which cadence runs a watcher is eligible for; a pack may set `defaults.cycles`. `schedule` (`daily | weekly | monthly | ...`) and `capture_mode` are first-class row fields a pack supplies as defaults and a row may override.
- **`capture_mode: manual` never auto-runs.** A cadence-triggered `check` may detect, but it never dispatches a manual watcher's harvest; only an explicit user-invoked `watchlist harvest --id <id>` may. Nothing in the tool widens `manual` to `automatic` or shortens `schedule` on its own initiative, and the 0.19.0 migration's validate step asserts each folded source kept its pre-migration cadence.
