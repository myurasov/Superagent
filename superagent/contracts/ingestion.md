...
- **Registry.** A source is one watcher file, `Sources/Watchlist/<name>.ref.md` (folder overridable via `config.preferences.watchlist.path`), `ref_version: 2`: title, cross-references, provenance, and a `watch:` block carrying the pack or type, the locator, and the cadence fields. The filename stem lowercased is the source id, its state key and its handle (`watch:<id>`). There is no Python table of sources and no central YAML registry.
