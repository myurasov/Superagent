...
| **registry** | The folder `Sources/Watchlist/` (`config.preferences.watchlist.path`): one `<name>.ref.md` per watcher; user-editable (§ 2). Every `.ref.md` under `Sources/` is a watcher — there is no other kind of ref (`contracts/sources.md` § 15.4). |
...
## 2. Registry — `Sources/Watchlist/<name>.ref.md`
...
**Filename and id.** The file is `<name>.ref.md`: the `.ref.md` suffix is what makes it a watcher, and the name is the user's — `HA.ref.md`, `ha.ref.md` and `Home_Assistant.ref.md` are all valid and are kept exactly as written; the tool never renames a user-authored file or warns about its casing. The names the **tool** generates are `Title_Case` — the first letter of every `_`- or `-`-delimited token capitalized: `enable --id gmail-bills` writes `Gmail-Bills.ref.md`, `--id simplefin` writes `Simplefin.ref.md`, `--id home_assistant-hub` writes `Home_Assistant-Hub.ref.md`. That is a convention for generated names, not a rule for the user's. **The `id` is the stem lowercased** (`simplefin`, `gmail-bills`, `home_assistant-hub`, `ha`); it is the state key and the handle (`watch:<id>`) and matches `^[a-z0-9][a-z0-9_-]{0,62}$` (lowercase, `_` between words, `-` inside tokens; never forced to kebab-case). Files resolve case-insensitively (`simplefin.ref.md` and `Simplefin.ref.md` are the same watcher); two files whose lowercased stems collide are a load error. Renaming a file to a different stem renames the watcher: the tool warns on the orphaned state row and offers to carry it across; `title` may change freely. Absent folder = feature off: `check` exits 0 with no output. `Sources/Watchlist/README.md` (from `templates/folder-readmes/Watchlist.md`) is documentation, not a watcher, and is excluded from indexing like `Sources/README.md`. Any other file in the registry folder that does not end in `.ref.md` is not a watcher either: the loader and `sources_index refresh` warn — "not a .ref.md — not a watcher; rename it to `<name>.ref.md` or move it out of the registry" (the hint keeps the user's own stem: `HA.md` → `HA.ref.md`) — and skip it.
