...
| AP-6 | info | RETIRED (0.8.0 — briefing-cache feature pruned; id kept for numbering stability). | — |
...
| AP-14 | warning | Reading every partition of the events stream (`_memory/events/<YYYY-Qn>.yaml`) for a timeline question. | Route through `uv run python -m superagent.tools.log_window read --since <date>` (loads only the partitions the window touches) per `rules/token-economy.md` § Unbounded-file decision table (events row). |
| AP-15 | warning | Whole-file read of a Domain / Project `history.md` (multi-year, unbounded) for a single-event question. | Grep `history.md` for the entity / date, then `Read --offset --limit` the matching slice; tail read for the recent window. Whole-file reads are for short, young histories only. |
| AP-16 | warning | Re-reading your own write to prove it landed (re-read / read-back / read-again forms). | The Edit / Write tool result already proves the change landed — reuse the evidence in context instead of re-fetching the file. Fall back to a count or exit code (`grep -c`, `yaml.safe_load` in a tool) only when the write's correctness is genuinely in doubt, per `rules/token-economy.md` § Always-on floor ("Never re-read your own writes"). |
