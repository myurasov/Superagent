...
3. It **asks the user** before writing:
   ...
   ```
4. The user's choice is held for the rest of the session (so they aren't re-asked for a batch). The persistent default lives in `config.preferences.sources.normalize_policy` (default `ask`).
