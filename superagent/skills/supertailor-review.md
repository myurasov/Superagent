...
| # | Step | Lines |
|---|------|-------|
| 1 | Hygiene pass | 51-69 |
| 2 | Strategic pass | 70-84 |
| 3 | Consult improvement-ideas catalogues | 85-128 |
| 4 | Destination classification | 129-141 |
| 5 | Report | 142-150 |
| 6 | Hand-off | 151-156 |
| 7 | Update memory | 157-162 |
...
## 3. Consult improvement-ideas catalogues
...
### 3b. Match friction → catalogue entry
...
|---|---|
| User keeps running 3 skills in sequence ("morning routine") | `superagent/docs/_internal/ideas-better-structure.md` § #21 (Skill bundles / playbooks) — **retired 0.8.0** |
| User asks "what changed since I last looked" | `superagent/docs/_internal/ideas-better-structure.md` § #6 (Differential snapshots) — **retired 0.8.0** |
...
| Same skill re-reads same files multiple times in a session | `superagent/docs/_internal/perf-improvement-ideas.md` § MI-1 (per-session scratchpad) — **retired 0.8.0** |
| Daily-update is regenerated for every "remind me what's today" follow-up | `superagent/docs/_internal/perf-improvement-ideas.md` § QW-5 (briefing cache) — **retired 0.8.0** |
| Sources cache hit rate is low | `superagent/docs/_internal/perf-improvement-ideas.md` § QW-6 (real LLM `_summary.md`) |

Entries marked **retired 0.8.0** were shipped and then pruned for zero usage (`docs/roadmap.md` § Released, 0.8.0). Before citing any catalogue match, check the Released table for a prior retirement; if found, the suggestion MUST cite the prune in `evidence` and show fresh usage signal before re-proposing — never re-propose a pruned feature on the catalogue entry alone.
