...
| # | Step | Lines |
|---|------|-------|
| 1 | Mode 1: render-all (default) | 51-70 |
| 2 | Mode 2: render one workbook | 71-81 |
| 3 | Mode 3: status | 82-91 |
| 4 | Mode 4: enable / disable opt-in workbooks | 92-103 |
| 5 | Mode 5: dry-run / check | 104-114 |
| 6 | Privacy scan | 115-132 |
| 7 | History window | 133-140 |
| 8 | Where workbooks live | 141-154 |
| 9 | Logging | 155-168 |
...
Append a canonical row to `_memory/interaction-log.yaml` (shape per `templates/memory/interaction-log.yaml`; mint the id with `uv run python -m superagent.tools.next_id --kind ilog --file <workspace>/_memory/interaction-log.yaml`):

```yaml
- id: "<ilog-YYYY-MM-DD-NNN>"
  ts: <now, ISO 8601 with offset>
  skill: "workbooks"
  action: "render_workbooks"
  summary: |
