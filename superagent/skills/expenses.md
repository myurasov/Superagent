...
mcp_required: []
mcp_optional:
  - finance ingestors (simplefin / plaid / monarch / ynab / csv) — required for any data
cli_required: []
cli_optional: []
---
...
> "No finance data ingested yet. Set up `simplefin`, `plaid`, `monarch`, `ynab`, or the `csv` ingestor first — see `superagent/docs/data-sources.md#finance`."
...
2. `_memory/transactions.yaml` — cross-account ingestor-normalized output (SimpleFin is the canonical writer; Plaid / Monarch / YNAB / CSV write the same shape).
3. Live ingestor pull (`uv run python -m superagent.tools.ingest.simplefin`) only when the question's window extends past the source's `last_ingest` in `data-sources.yaml`. A bank-portal `browserctl` session is a last resort — only when the user explicitly asks or the account is not covered by any ingestor — and is announced as such.
