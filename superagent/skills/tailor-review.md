---
name: superagent-tailor-review
description: >-
  Run the Tailor's two-pass review (hygiene + strategic). Hygiene applies
  mechanical reversible repairs to the workspace. Strategic surfaces
  ranked framework-improvement suggestions and writes them to
  `pm-suggestions.yaml`, tagged `superagent` (handed to Coder) or
  `_custom` (Tailor implements directly).
triggers:
  - tailor review
  - run the tailor
  - what should I improve in superagent
  - audit the framework
  - is anything broken in superagent
mcp_required: []
mcp_optional: []
cli_required: []
cli_optional: []
---



## Step index

_Auto-generated by `tools/add_step_index.py`. Re-run on any skill edit._

| # | Step | Lines |
|---|------|-------|
| 1 | Hygiene pass | 25-40 |
| 2 | Strategic pass | 42-55 |
| 3 | 2½. Consult improvement-ideas catalogues | 57-97 |
| 3 | Destination classification | 99-110 |
| 4 | Report | 112-119 |
| 5 | Hand-off | 121-125 |
| 6 | Update memory | 127-131 |

# Superagent tailor-review skill

This is the runnable shell of the Tailor agent. The full role definition is in `tailor.agent.md`. The destination-classification + safeguard contracts are referenced from there.

## 1. Hygiene pass

Mechanical, reversible repairs.

Per `tailor.agent.md` § "Hygiene pass":

1. **Template compliance**: every `Domains/<domain>/` has the four files; every file carries the maintenance banner; every `history.md` has the right `## Log` shape with H4 date headers; every `status.md` has `## Status / ## Open / ## Done` blocks.
2. **Orphan detection**: folders with no index row; index rows pointing nowhere; cross-references to non-existent ids.
3. **Memory staleness**: `context.yaml.last_check` older than 7 days; `model-context.yaml` not updated this month; cadence skill stale per the documented intervals.
4. **Cadence adherence**: surface "you said weekly review weekly but the last one was 23 days ago — drop the cadence preference, or is something off?".
5. **Schema integrity**: validate every `_memory/*.yaml` against its `schema_version` (delegate to `tools/validate.py`).
6. **Ingestion-source health**: every `data-sources.yaml.<source>.failure_streak > 0` surfaced with the failure cause and one-line "what to fix".
7. **Custom-overlay scaffold**: ensure `_custom/{rules,skills,agents,templates,tools}/` exist; create empty if missing.
8. **Improvement-ideas catalogues exist**: verify `superagent/docs/ideas-better-structure.md` (expected tier headings: § 1 through § 25 + Selection guide) and `superagent/docs/perf-improvement-ideas.md` (expected tier headings: Quick wins / Medium investments / Big bets) are present and parseable. They are mandatory inputs to step 2.5 below; missing files silently degrade Tailor output. Surface as `needs-attention` (not auto-fixable — both are hand-curated).

For each finding, list it; ask `approve all / approve some / decline all`. Apply approved repairs; back up modified files into `_memory/_checkpoints/<date>/`.

## 2. Strategic pass

Pattern detection on usage data.

Per `tailor.agent.md` § "Strategic pass":

1. **Friction patterns** from `user-queries.jsonl` (clusters of similar queries) and `action-signals.yaml` (`target: tailor` rows still `status: captured`).
2. **Skill utilization** — skills not run in 30 days (deprecation candidates) and skills run with high frequency (optimization candidates).
3. **Domain utilization** — silent domains (archive candidates) and overactive domains (split candidates).
4. **Ingestor utilization** — sources that ingest but whose data is never queried (disable candidates) and not-configured sources for whose data the user keeps asking (recommend setup).
5. **Auto-capture rule misses** — manual signals captured shortly after an ingest run (likely missed auto-capture rules).
6. **Output / template critiques** from `action-signals.yaml.kind: artifact-critique`.

Each strategic finding becomes a row in `_memory/pm-suggestions.yaml` with full context (problem, evidence, suggestion, implementation_sketch, effort, risk, destination).

## 2½. Consult improvement-ideas catalogues

Before drafting any new suggestion, the Tailor MUST check whether the friction patterns from step 2 match a pre-analyzed entry in either catalogue. This avoids re-deriving ideas the framework has already thought through, and surfaces "this isn't a one-off; it's been catalogued since <date>".

### 2½a. Read the catalogues

1. **`superagent/docs/ideas-better-structure.md`** — 25 structural-improvement options, each tagged Mechanical / Schema / Conceptual / Operational, with LOE / trade-off / "when to do" guidance.
2. **`superagent/docs/perf-improvement-ideas.md`** — token-efficiency / cache-hit / latency improvements, tiered Quick wins (QW-1 to QW-7) → Medium investments (MI-1 to MI-5) → Big bets (BB-1 to BB-4).

Use a TOC-first read pattern: read the section index, then jump only to the relevant tier sections. Do not load whole catalogues into context.

### 2½b. Match friction → catalogue entry

For each friction theme from step 2, check whether a catalogue entry addresses it. Examples:

| Friction signal | Likely catalogue match |
|---|---|
| User keeps running 3 skills in sequence ("morning routine") | `ideas-better-structure.md` § #21 (Skill bundles / playbooks) |
| User asks "what changed since I last looked" | `ideas-better-structure.md` § #6 (Differential snapshots) |
| User says "find that note about X" repeatedly | `ideas-better-structure.md` § #7 / § #8 (FTS5 / embeddings) |
| User wants cross-cutting view by tag | `ideas-better-structure.md` § #11 (Tagging as first-class) |
| User asks "where did this fact come from?" | `ideas-better-structure.md` § #4 (Provenance + facts-with-sources) |
| Agent feels slow / token costs are high | `perf-improvement-ideas.md` § Quick wins |
| Same skill re-reads same files multiple times in a session | `perf-improvement-ideas.md` § MI-1 (per-session scratchpad) |
| Daily-update is regenerated for every "remind me what's today" follow-up | `perf-improvement-ideas.md` § QW-5 (briefing cache) |
| Sources cache hit rate is low | `perf-improvement-ideas.md` § QW-6 (real LLM `_summary.md`) |

### 2½c. Cite the match in the suggestion

When a friction theme matches a catalogue entry, the resulting `pm-suggestions.yaml` row MUST cite it:

- `evidence`: append `" — matches catalogue: <doc-short-name> § <id> (<title>)"`. Example: `"4 user-queries.jsonl entries this week of the form 'what changed since…' — matches catalogue: ideas-better-structure § #6 (Differential snapshots)"`.
- `implementation_sketch`: prefer pointing at the catalogue's existing sketch over re-deriving it.

### 2½d. Surface gaps

If a friction theme matches NO catalogue entry, note it in the report under `### Catalogue gaps` with a one-line description. Do NOT auto-add to the catalogue (it's hand-curated). The user can promote during the review or ignore.

### 2½e. Do not mutate the catalogues

The catalogues are READ-ONLY for the Tailor. New ideas surfaced here go into `pm-suggestions.yaml` as normal; promoting them into the catalogue is a manual user step.

## 3. Destination classification

For every suggestion before write:

1. Tailor classifies: `destination: superagent` (generic) or `_custom` (user-specific).
2. **Token-scan safeguard** — scan suggestion body against:
   - `_memory/contacts.yaml.contacts[].name` and `.aliases[]`
   - `_memory/domains-index.yaml.<domain>.id`
   - `_memory/assets-index.yaml.<asset>.name` and `.serial`
   - `_memory/accounts-index.yaml.<acct>.name`
   - Address-fragment / license-plate / account-number regex patterns from those same files
3. On any match, force `destination: _custom`. Add `implementation_notes: "Auto-routed to _custom/ — safeguard matched: <token>"`.

## 4. Report

Per `tailor.agent.md` § "Report format". The report includes:

- Hygiene-pass results (what was checked, what was repaired, what was skipped).
- Strategic-pass results (count of new suggestions by destination, count of pending, declined, deferred).
- Routing summary (how many were forced from `superagent` to `_custom` by the safeguard, and what tokens triggered them).
- Quick action: for each new suggestion, ask `approve / edit / defer / decline`.

## 5. Hand-off

For approved suggestions:
- `destination: _custom` → Tailor implements directly into `workspace/_custom/`. Sets `status: implemented`, `implementation_notes: "<file path + summary>"`.
- `destination: superagent` → Tailor packages a brief and hands to the Coder ("Coder, implement pm-NNN per the brief"). The Coder re-runs the safeguard, implements into `superagent/`, runs `pytest`, commits, reports back. Tailor flips status to `implemented` with the commit SHA.

## 6. Update memory

1. `_memory/context.yaml.last_runs.tailor_review` → now.
2. Append to `interaction-log.yaml`.
3. `pm-suggestions.yaml` updated with the new rows.
4. `_memory/_checkpoints/<date>/` populated with backups of any files modified by hygiene-pass repairs.
