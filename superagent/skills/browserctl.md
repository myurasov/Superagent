...
| # | Step | Lines |
|---|------|-------|
| 1 | Step 0 — Why this exists | 25-41 |
| 2 | Step 1 — Profiles: per-project, clean, ephemeral on demand | 43-67 |
...
| 7 | Step 6 — Hard rules and known constraints | 158-174 |
...
| `icon [--png F\|--icns F] [--color C] [--name "Superagent Browser"]` | macOS: build the dedicated "Superagent Browser" app bundle (own Dock name + icon, default teal; APFS clone of Playwright's Chromium, ad-hoc re-signed). Once built, every launch uses it; relaunch running profiles to pick it up. Rebuild after `playwright install chromium` updates. |
