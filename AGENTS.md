...
- [`superagent` workspace](#superagent-workspace)
  ...
  - [IDE setup (Cursor / Claude Code)](#ide-setup-cursor--claude-code)
  - [Prompt-cache discipline](#prompt-cache-discipline)

---

This file is the **canonical, IDE-agnostic always-on instructions** for **Superagent** — a personal-life AI assistant. The framework code lives in the `superagent/` folder; user data lives in the sibling `workspace/` folder. Whenever the user invokes Superagent (or works within `workspace/`), the agent reads this file and follows it in full.
...
- `Made-with: Cursor`
- `Co-Authored-By: Claude <…>` / any `Co-Authored-By:` trailer naming an AI agent or vendor
- `Generated with [Claude Code]` / `Generated with [Cursor]` / `Generated by AI` / `Created with AI` / `Made with ChatGPT`
- robot / sparkles emoji indicating AI origin
- `"via Cursor"` / `"via Claude Code"` / `"with Claude"` / similar attributions
...
Superagent is IDE-agnostic. It works in Cursor, Claude Code, or any other AI coding assistant that can read files and follow instructions. It deliberately ships **without** auto-injection rules — the user opts in to Superagent per session by invoking a skill or by saying so. This avoids polluting other work the user might be doing in the same workspace.

Optional IDE wiring (only relevant if Superagent is the primary framework in the repo or if the user wants its rules auto-injected):
...
Per `docs/perf-improvement-ideas.md` BB-2-b — practical guidance for Cursor / Claude Code today:
