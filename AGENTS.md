...
- **Email capture** (mirrors touched Gmail messages into the local archive). Cursor: `.cursor/hooks.json`, event `afterMCPExecution`, ONE entry with no matcher, `archive_hook --kind=auto`. Claude Code: `.claude/settings.json`, event `PostToolUse`, three `mcp__gmail__<tool>` matchers. Details and the per-harness field-name differences are in [`contracts/email-capture.md`](superagent/contracts/email-capture.md) § 8.1; the hook-free floor that makes both optional is `rules/email-capture-fallback.md`.
